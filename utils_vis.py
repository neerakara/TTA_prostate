...
def save_sample_results(x,
    ...
    for i in range(len(ids)): 
        ...
        plt.subplot(len(ids), nc, nc*i+3); plt.imshow(y_pred_[ids[i],:,:], cmap='tab20'); plt.colorbar(); plt.title('pred')
        plt.subplot(len(ids), nc, nc*i+4); plt.imshow(y_pred_dae_[ids[i],:,:], cmap='tab20'); plt.colorbar(); plt.title('dae output')
